"""
Strategy 4 - test.py
python3 -m src.parsers.strategy_4_lexicon.test | tee src/parsers/strategy_4_lexicon/output.txt
"""
...
from src.parsers.strategy_4_lexicon.post_processor import clean_and_chunk
...
def test_single_pdf(pdf_path: Path, max_pages: int = None):
    print_header("LEXICON PARSER TEST")
...
def select_pdf() -> Path:
    """
    Prompt user to select a PDF file from data/strategy_4_lexicon/.
    
    Returns:
        Path to selected PDF
    """
    data_dir = Path("data/strategy_4_lexicon")
