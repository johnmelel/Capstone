"""
Strategy 1 - test.py
python3 -m src.parsers.strategy_1_clinical_image.test | tee src/parsers/strategy_1_clinical_image/output.txt
"""
...
from src.parsers.strategy_1_clinical_image.post_processor import clean_and_chunk
...
def display_chunk(chunk: Dict, index: int):
    ...
    if chunk['type'] == 'text':
        ...
        print(f"\n{content}")
        print(f"{'. . ' * 80}")
        print(f"\nLength: {len(content)} chars, {len(content.split())} words")
        ...
        print(f"Table content: {chunk.get('content', '')[:]}...")
        if chunk.get('caption'):
            print(f"Caption: {chunk['caption']}")
...
def select_pdf() -> Path:
    """
    Prompt user to select a PDF file from data/strategy_1_clinical_image/.
    
    Returns:
        Path to selected PDF
    """
    data_dir = Path("data/strategy_1_clinical_image")
