...
- starttime (TEXT): When prescription started
...
emar table:
- subject_id (INTEGER): Patient identifier
- charttime (TEXT): When medication was administered
...
LIMIT 50;"""
            ...
            {
                "query": "List 5 medications people took in the past 12 hours",
                "sql": """SELECT DISTINCT 
    pr.drug,
    COUNT(DISTINCT pr.subject_id) as patient_count,
    pr.starttime
FROM prescriptions pr
WHERE pr.starttime >= datetime('now', '-12 hours')
GROUP BY pr.drug
